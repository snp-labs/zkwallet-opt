...
if (index.snapshots.length < 2) {
  throw new Error("Need at least two smoke export snapshots to check regression history delta.");
}
...
if (!latest) {
  throw new Error("Could not resolve the requested latest snapshot for regression history check.");
}

if (!previous) {
  throw new Error(
    "Could not resolve the requested previous snapshot for regression history check."
  );
}
...
const comparisonPayload = buildRegressionsHistoryComparisonPayload(
  baseDir,
  latest,
  previous,
  latestHistory,
...
const payload = buildRegressionsHistoryComparisonCheckPayload(comparisonPayload);

if (jsonOnly) {
  console.log(JSON.stringify(payload, null, 2));
} else {
  process.stdout.write(renderRegressionsHistoryComparisonCheckText(payload));
}
