...
async function main() {
  ...
  if (index.snapshots.length < 2) {
    throw new Error("Need at least two smoke export snapshots to compare.");
  }
  ...
  const latestChecksums = readJson(path.join(latest.snapshotDir, "checksums.json"));
  const previousChecksums = readJson(path.join(previous.snapshotDir, "checksums.json"));
  ...
  if (jsonOnly) {
    ...
    process.stdout.write(renderComparisonText(payload));
  }
}
