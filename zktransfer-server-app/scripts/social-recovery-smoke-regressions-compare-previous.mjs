...
if (index.snapshots.length < 2) {
  throw new Error(
    "Need at least two smoke export snapshots to open the previous regressions compare."
  );
}

const snapshot = index.snapshots[1];
const comparePath = path.join(snapshot.snapshotDir, "regressions-compare.txt");
const compareJsonPath = path.join(snapshot.snapshotDir, "regressions-compare.json");

if (!fs.existsSync(comparePath)) {
  throw new Error(`Missing previous snapshot regressions compare artifact at ${comparePath}.`);
}
