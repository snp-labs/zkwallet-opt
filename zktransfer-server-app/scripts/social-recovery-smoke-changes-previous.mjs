...
if (index.snapshots.length < 2) {
  throw new Error("Need at least two smoke export snapshots to open the previous changes.");
}

const snapshot = index.snapshots[1];
const changesTextPath = path.join(snapshot.snapshotDir, "changes.txt");
const changesJsonPath = path.join(snapshot.snapshotDir, "changes.json");

if (!fs.existsSync(changesTextPath)) {
  throw new Error(`Missing previous snapshot changes artifact at ${changesTextPath}.`);
}
